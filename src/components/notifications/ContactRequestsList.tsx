...
export function NotificationsList({ onClose }: NotificationsListProps) {
  const { contactRequests, loading, error: notifError, markAsRead } = useNotifications();
  const [selectedRequest, setSelectedRequest] = React.useState<string>();
  const { user } = useAuth();
  const navigate = useNavigate();

  // Filtrer pour n'afficher que les demandes de contact reçues
  const receivedRequests = contactRequests;
  ...
  return (
    <div className="divide-y divide-pink-100">
      {receivedRequests.map((notification: Notification) => (
        <div
          key={notification.id}
